...
class manage_productController extends Controller
{
    public function product_list(Request $request)
    {
        ...
        return view('admin.manage_product', compact('categories')); // Pass to view
    }
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($products)
                ...
                ->addColumn('product_image', function ($row) {
                    return '<img src="' . asset('storage/' . $row->pro_img) . '" width="50">';
                })
                ->addColumn('actions', function ($row) {
                    return ' <button class="btn btn-sm btn-warning editProductBtn"
                        data-id="' . base64_encode($row->id) . '"
                        data-bs-toggle="modal"
                        data-bs-target="#producteditModal">
                        <i class="fa fa-edit"></i>
                    </button>
                   <button class="btn btn-sm btn-danger delete-product" data-id="' . base64_encode($row->id) . '">
